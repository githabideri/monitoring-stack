...
```promql
# Can I reach the offsite LAN via Tailscale right now?
# 1 = yes, 0 = a leg is broken, ABSENT = the check itself is not running.
homelab:tailscale_offsite_healthy
...
# Which tailnet peers are offline (watch list only)?
tailscale_peer_online == 0
```
